 ...
 * initializing the Android Keystore auth store. 401-replay/relogin paths route through
 * login() (which reads AuthPrefs) and are covered by the end-to-end verification, not here.
 */
class EgoFlowBackendClientHttpStreamTest {
    ...
    @Test
    fun httpStreamChunk_usesTwentySecondReadTimeout() {
        val chunkClient = createHttpChunkClient(OkHttpClient())

        assertEquals(20_000, chunkClient.readTimeoutMillis)
    }
