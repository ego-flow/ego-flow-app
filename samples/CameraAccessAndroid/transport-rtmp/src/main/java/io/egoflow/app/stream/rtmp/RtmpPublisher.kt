...
class RtmpPublisher(
    ...
    private var hasPublishedVideoSample = false
    ...
    fun sendVideoConfig(videoCodec: RtmpVideoCodec, format: MediaFormat) {
        pendingVideoConfig = RtmpVideoPacketizer.buildSequenceHeader(videoCodec, format)
        hasPublishedVideoSample = false
        waitingForVideoKeyFrame = true
    ...
    fun sendVideoSample(videoCodec: RtmpVideoCodec, buffer: ByteArray, info: MediaCodec.BufferInfo): Boolean {
        if (info.size <= 0 || info.flags and MediaCodec.BUFFER_FLAG_CODEC_CONFIG != 0) return false
        val isKeyFrame = info.flags and MediaCodec.BUFFER_FLAG_KEY_FRAME != 0
        if (waitingForVideoKeyFrame && !isKeyFrame) return false
        val timestampMs = (info.presentationTimeUs / 1000L).toInt()
        if (pendingVideoConfig != null) {
            sendVideoMessage(requireNotNull(pendingVideoConfig), timestampMs)
            pendingVideoConfig = null
        }
        waitingForVideoKeyFrame = false
        hasPublishedVideoSample = true
        val compositionTimeMs = 0
        val packet = RtmpVideoPacketizer.buildVideoPacket(videoCodec, buffer, isKeyFrame, compositionTimeMs)
    ...
    fun sendAudioSample(buffer: ByteArray, info: MediaCodec.BufferInfo): Boolean {
        if (!hasPublishedVideoSample) return false
        if (info.size <= 0 || info.flags and MediaCodec.BUFFER_FLAG_CODEC_CONFIG != 0) return false
        val timestampMs = (info.presentationTimeUs / 1000L).toInt()
