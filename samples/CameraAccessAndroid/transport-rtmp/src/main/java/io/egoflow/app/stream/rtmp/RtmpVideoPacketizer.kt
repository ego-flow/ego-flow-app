...
internal object RtmpVideoPacketizer {
    ...
    fun buildSequenceHeader(videoCodec: RtmpVideoCodec, format: MediaFormat): ByteArray {
        return when (videoCodec) {
            RtmpVideoCodec.H264 -> {
                ...
                buildAvcSequenceHeader(sps = sps, pps = pps)
            }

            RtmpVideoCodec.H265 -> {
                val csd = format.getByteBuffer("csd-0") ?: error("HEVC csd-0 is missing")
                val nalUnits = extractNalUnits(ByteArray(csd.remaining()).also { csd.get(it) })
                ...
                buildHevcSequenceHeader(vpsUnits, spsUnits, ppsUnits)
            }
        }
    }
    ...
    fun buildVideoPacket(
        ...
        return when (videoCodec) {
            ...
            RtmpVideoCodec.H265 -> buildHevcVideoPacket(annexBData, isKeyFrame, compositionTimeMs)
        }
    }
