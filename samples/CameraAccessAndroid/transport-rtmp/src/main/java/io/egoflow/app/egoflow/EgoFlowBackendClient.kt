...
class EgoFlowBackendClient(
    ...
    // without standing up EncryptedSharedPreferences. Production callers use the
    ...
    private fun normalizeApiBaseUrl(raw: String): okhttp3.HttpUrl {
        val withScheme =
            when {
                ...
                // dev hosts, but is gated by the network-security-config allowlist at runtime.
                else -> "https://$raw"
            }
    ...
    private inline fun <reified T> executeJsonResponse(request: Request): JsonResponse<T> {
