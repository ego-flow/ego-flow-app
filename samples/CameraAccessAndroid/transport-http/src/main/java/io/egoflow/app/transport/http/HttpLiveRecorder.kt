...
class HttpLiveRecorder(
    ...
    fun queueGlassesFrame(bytes: ByteArray, width: Int, height: Int, sourcePtsUs: Long) {
        submit {
            if (!ensureStarted(width, height)) return@submit
            encoder.queue(bytes, width, height, glassesPtsUs(sourcePtsUs))
    ...
    fun queuePhoneFrame(i420: ByteArray, width: Int, height: Int) {
        submit {
            if (!ensureStarted(width, height)) return@submit
            encoder.queue(i420, width, height, phonePtsUs(System.nanoTime()))
            drain(endOfStream = false)
        }
    }
